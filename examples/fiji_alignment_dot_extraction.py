...
class alignImages():
    def __init__(self, inputFolder, outputFolder, stringFilter, useInterpolate, useShrinking):
        ...
        for folder in [self.inputDAPIFolder, self.inputFluoFolder, self.outputDAPIFolder, self.outputFluoFolder]:
    ...
    def getFiles(self, inputFolder):
        ...
        for DAPIFile in DAPIFiles:
            fluoFiles = [f for f in glob.glob(os.path.dirname(DAPIFile) + "/*.tif") if f != DAPIFile]
    ...
    def runVirtualAlignment(self):
        ...
        for (DAPIFile, fluoFiles) in self.imageFiles:
            ...
            if refImage is None:
                refImage = os.path.basename(self.inputDAPIFolder + os.path.basename(DAPIFile))
        
        p = Register_Virtual_Stack_MT.Param()
        use_shrinking_constraint = self.useShrinking
        
        IJ.log("Registering DAPI")
        Register_Virtual_Stack_MT.exec(self.inputDAPIFolder, self.outputDAPIFolder, self.outputDAPIFolder, refImage, p, use_shrinking_constraint)

        IJ.log("Copying DAPI transformation matrices to Fluo folder")
        for (DAPIFile, fluoFiles) in self.imageFiles:
            for fluoFile in fluoFiles:
                copyfile(self.outputDAPIFolder + os.path.basename(DAPIFile).replace(".tif",".xml"), self.outputFluoFolder + os.path.basename(fluoFile).replace(".tif",".xml"))
        
        use_interpolate = self.useInterpolate
        IJ.log("Registering Fluo based on DAPI transformations")
        Transform_Virtual_Stack_MT.exec(self.inputFluoFolder, self.outputFluoFolder, self.outputFluoFolder, use_interpolate)
...
class extractDots ():
    def __init__(self, imageFilename, gaussianVal, tophatSize, trackmateSize, trackmateThreashold):
    ...
    def preprocess (self):
        ...
        IJ.run(self.imp, "Gray Morphology", "radius="+str(tophatSize)+" type=circle operator=open");
    ...
    def spotsToCSV (self, spots):
        ...
        qmin, qmax = min([s["q"] for s in spotsDict]), max([s["q"] for s in spotsDict])/3.
    ...
    def transformSpots(self, spotsDict, boundingBox, transFile):
        ...
        for spot in newDict:
            ...
            spot["x"], spot["y"] = spot["x"] - boundingBox[0], spot["y"] - boundingBox[2]
        return newDict
...
class TissUUmaps():
    ...
    def getJsonProject(self):
        return {
            "markerFiles": [
                        {
                            ...
                            "title": "Download markers","comment": "Only markers with q>{q}".format(q=qualityThreshold),
                            "expectedCSV": { 
                                "group": "name", "name": "", "X_col": "x", "Y_col": "y", "key": "letters", "color": "color"
                            },
                            "autoLoad": True
                        }
                    ], 
            ...
            "filename": "",
            "settings": [
                {"module":"overlayUtils","function":"_linkMarkersToChannels","value":True},
...
ai = alignImages(unicode(inputFolder), unicode(outputFolder), stringFilter, useInterpolate, useShrinking)
...
for imageFilename in images:
    ...
    eD[imageFilename] = extractDots (imageFilename, gaussianVal, tophatSize, trackmateSize, trackmateThreashold)
...
for imageFilename in images:
    ...
    transFile = imageFilename.replace("fluo","fluo_registered").replace(".tif",".xml")
    transDict = eD[imageFilename].transformSpots(eD[imageFilename].csvSpots, boundingBox, transFile)
...
with open(unicode(outputFolder) + "/markers_{q}.csv".format(q=qualityThreshold), 'wb') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=allSpotsDict[0].keys())
    writer.writeheader()
    for data in allSpotsDict:
...
with open(unicode(outputFolder) + "/markers_reg_{q}.csv".format(q=qualityThreshold), 'wb') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=allSpotsTransDict[0].keys())
    writer.writeheader()
    for data in allSpotsTransDict:
        if data["q"]>qualityThreshold:
...
images = glob.glob(unicode(outputFolder) + "/DAPI/*.tif") + glob.glob(unicode(outputFolder) + "/fluo/*.tif")
layers = [os.path.basename(os.path.dirname(i)) + "/" + os.path.basename(i) + ".dzi" for i in images]
TissUUmaps(layers, ["markers.csv","markers_{q}.csv".format(q=qualityThreshold)], unicode(outputFolder)+"/NonRegistered.tmap")
images = glob.glob(unicode(outputFolder) + "/DAPI_registered/*.tif") + glob.glob(unicode(outputFolder) + "/fluo_registered/*.tif")
layers = [os.path.basename(os.path.dirname(i)) + "/" + os.path.basename(i) + ".dzi" for i in images]
TissUUmaps(layers, ["markers_reg.csv","markers_reg_{q}.csv".format(q=qualityThreshold)], unicode(outputFolder)+"/Registered.tmap")
