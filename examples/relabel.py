...
def loadDBIntoDataFrame(dbfilename):
    ...
    df_po=None;df_so=None;df_im=None
...
def drawNeighbors(im,ir,dfm,dfr,neighbor,overlap,grace):
    ...
    for xx,yy in zip(x,y):
        ...
        axarr[0].annotate(str(count), (xx,yy), color='w', weight='bold',fontsize=6, ha='center', va='center')
        count+=1
    ...
    for xx,yy in zip(x,y):
        ...
        axarr[1].annotate(str(count), (xx,yy), color='w', weight='bold',fontsize=6, ha='center', va='center')
        count+=1
...
def pointDist(p1,p2,xoffset=0,yoffset=0):
    ...
    #print (x1,x2+xoffset)
    #print (y1,y2+yoffset)
    d=np.sqrt((x1-(x2+xoffset))*(x1-(x2+xoffset))+(y1-(y2+yoffset))*(y1-(y2+yoffset)))
    return d
            ...
            for ri,rb in nborder.iterrows():
                ...
                if rb["Matched"]==-1:
                    ...
                    if d<maxdistance:
                        ...
                        df_nuclei_mod.at[mi,"Matched"]=nborder.at[ri,"Global_Exp_ID"]
                        df_nuclei_mod.at[ri,"Matched"]=mborder.at[mi,"Global_Exp_ID"]
    return mborder, nborder

def goThroughNeighbor (mobjects,robjects,inm,inr,neighbor,tilesize,overlap,grace=2,usesob=False,draw=False,debug=False):
    """
    ...
    yoff=0
    if neighbor=="right":
        loc=po_x_location;feature=tilewidth;xoff=tilesize;yoff=0
    if neighbor=="bottom":
        loc=po_y_location;feature=tileheight;xoff=0;yoff=tilesize
    ...
    mborder,rborder = matchDFs(mborder,rborder,xoffset=xoff,yoffset=yoff,maxdistance=10)
    ...
    for mi,mb in matchedm.iterrows():
        ...
        mf=mb[loc]; rf=rb[loc].values[0]
        md=mfeature-mf; rd=rf
        ...
        if md<rd:
            ...
            df_nuclei_mod.at[ri,"Erase"]=True
            if printinfo:
                print( str(count)+"md>rd m is further away from the border in "+neighbor)
            continue
        
def matchObjectsInBorders(x,y,mainlocation,tilesize=1024,overlap=100,grace=2.1,filetypeorig=".png",filetypelabel=".png",suffix="labels",draw=True):
    """ Matches all the objects in the borders of tiles """
    
    images={"main":None, "bottom":None, "right":None, "diagonal":None}
    file_nuc_bases={"bottom":str(x)+"_"+str(y+1), "right":str(x+1)+"_"+str(y)}
    ...
    if os.path.isfile(path):
        ...
        ImageNumbers["main"]=df_image.loc[df_image[filenamecolumn]==str(x)+"_"+str(y)+filetypeorig]["ImageNumber"].values[0]
    else:
    ...
    for n in hasNeighbor:
        ...
        if os.path.isfile(path):
            ...
            vals=df_image.loc[df_image[filenamecolumn]==file_nuc_bases[n]+filetypeorig]["ImageNumber"].values
            if(vals.size > 0):
                #print (vals)
            ...
            mobjects=df_nuclei_mod.loc[df_nuclei_mod["ImageNumber"]==ImageNumbers["main"]]
            bobjects=df_nuclei_mod.loc[df_nuclei_mod["ImageNumber"]==ImageNumbers[n]]
            ...
            for index, row in mobjects.iterrows():
                ...
                df_nuclei_mod.at[index,"Global_X"]=row[po_x_location]+x*tilesize-overlap
                #if (df_nuclei_mod.at[index,"Global_Y"]<0):
                df_nuclei_mod.at[index,"Global_Y"]=row[po_y_location]+y*tilesize-overlap
            ...
            goThroughNeighbor(mobjects,bobjects,ImageNumbers["main"],ImageNumbers[n],n,tilesize,overlap,grace=grace,draw=draw)

            if draw:
                mobjects=df_nuclei_mod.loc[df_nuclei_mod["ImageNumber"]==ImageNumbers["main"]]
                bobjects=df_nuclei_mod.loc[df_nuclei_mod["ImageNumber"]==ImageNumbers[n]]
                drawNeighbors(images["main"],images[n],mobjects,bobjects,n,overlap,grace)
            hasNeighbor[n]=True  
...
def eraseSelectedLabels(x,y,nl,vl,savelocation,filetypeorig=".png",filetypelabel=".png",nuc_sfx="labels",vor_sfx="voronoi",draw=False,save=False):
    filename=str(x)+"_"+str(y)+filetypeorig
    #get the ImageNumber
    inum=df_image.loc[df_image[filenamecolumn]==filename]["ImageNumber"].values[0]
    numplots=2;vor_image=None;    
    #get all nuclei belonging to inum
    nuclei=df_nuclei_mod.loc[
        (df_nuclei_mod["ImageNumber"]==inum) & 
        (df_nuclei_mod["Erase"]==True) ]
        #&
    ...
    nucleiNaN=df_nuclei_mod.loc[
        (df_nuclei_mod[po_x_location].isna()) |
        (df_nuclei_mod[po_y_location].isna())]
    ...
    if len(nuclei)>0:
        ...
        if save:
            io.imsave(savelocation+"labels/"+str(x)+"_"+str(y)+"corrected"+filetypelabel,nuc_image)
            if vl:
                io.imsave(savelocation+"voronoi/"+str(x)+"_"+str(y)+"corrected"+filetypelabel,vor_image)
        ...
        if save:
            try:
                ...
                print ("Could not read image. Return false and continue (x,y,inum,filename)")
                print (x,y,inum,nl+str(x)+"_"+str(y)+nuc_sfx+filetypelabel,vl+str(x)+"_"+str(y)+vor_sfx+filetypelabel)
                print (sys.exc_info()[0])
                return False
            io.imsave(savelocation+"labels/"+str(x)+"_"+str(y)+"corrected"+filetypelabel,nuc_image)
            if vl:
...
def unpadfolder(tilesize,overlap,mainlocation,savelocation,suffix,filetypeorig=".png",filetypelabel=".png",debug=False):
    i=0;j=0
    ...
    while(hasrightn or hasbottomn):        
        ...
        print("saving",i,j)
        io.imsave(savelocation+str(i)+"_"+str(j)+"border"+filetypelabel,savethis)
