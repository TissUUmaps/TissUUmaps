    ...
    if (typeof data != 'object') {
        ...
        $.each(els, function() {
            if (this.id && !this.disabled && (this.checked ||
                    ...
                    /text|hidden|password|color|range/i.test(this.type))) {
...
/** 
 * stores all form inputs' values into an object */
interfaceUtils.getFormValues = function() {
    formValues = $("body").values();
    return formValues;
}

/** 
 * @param {object} an objet with all form values 
 * Set all form inputs to corresponding values in the formValues object */
interfaceUtils.setFormValues = function(formValues) {
