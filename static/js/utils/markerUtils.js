...
markerUtils.drawCPdata = function(options) {
    ...
    if (searchInTree || false) {
        data = CPDataUtils.arrayOfElementsInBox(options.xmin, options.ymin, options.xmax, options.ymax, { xselector: xselector, yselector: yselector });
...
markerUtils.drawAllFromList = function(list) {
    ...
    console.log("OSDViewerUtils._currentZoom", OSDViewerUtils.getZoom(op));
    calculatedSize = Number(calculatedSize) / (OSDViewerUtils.getZoom(op) * OSDViewerUtils.getZoom(op));
...
markerUtils.markerUI = function(barObject, options) {
    ...
    if (shapeParams.random) {
        var rnd = Math.floor(Math.random() * (markerUtils._d3SymbolStrings.length)) + 0;
...
markerUtils.printBarcodeUIs = function(options) {
        ...
        var container = document.getElementById(op + "_markers");
        container.innerHTML = "";
