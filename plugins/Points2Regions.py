...
class Plugin ():
    ...
    def getCacheFile (self, jsonParam):
        ...
        return os.path.join( tempfile.gettempdir(), '_'.join( [ tempfile.gettempprefix(), "tmapPoints2Regions", hashKey ] ) )
...
def create_gene_signatures(
    ...
    gene_labels_numeric_unqiue = np.array([gene_labels_unique_map[str(label)] for label in gene_labels_unqiue])
...
def preprocess_vectors(gene_vectors, gene_vector_coords, threshold=None, logtform=True, normalize=True, ord=1):
    '''
...
def makejson(regionname: str, labels_numeric_unique, labels: np.array, xy:np.array, bin_stride, colors):
    ...
    for region in regionprops(labelmask):
        ...
        print(region.label)
        contours = binary_mask_to_polygon(region.image, offset=np.array([region.bbox[0]+0.5, region.bbox[1]+0.5]), scale=bin_stride)
...
def points2geojson(xy: np.array, labels: np.array, sigma: float, stride: int, region_name: str = 'My regions', nclusters: int = 4, normalize_order: int = 1, expression_threshold: float = 3):
    ...
    # Geojson
    json = makejson(region_name, gene_labels_unique, clusterlabels, gene_vector_coords, stride, colors)
...
def test():
    ...
    geojson = points2geojson(xy, labels, sigma, stride, region_name='My regions', nclusters=8, normalize_order=1, expression_threshold=3)
