...
Spot_Inspector.init = function (container) {
  ...
  label212 = HTMLElementUtils.createElement({
    ...
    extraAttributes: {
      class: "form-check-label",
      for: "Spot_Inspector_order_rounds",
    },
  });
  label212.innerHTML = "Round order:";
  var input212 = HTMLElementUtils.createElement({
    kind: "input",
    id: "Spot_Inspector_order_rounds",
    extraAttributes: {
      class: "form-text-input form-control",
      type: "text",
      value: JSON.stringify(Spot_Inspector._order_rounds),
    },
  });
  ...
  label312 = HTMLElementUtils.createElement({
    ...
    extraAttributes: {
      class: "form-check-label",
      for: "Spot_Inspector_order_channels",
    },
  });
  label312.innerHTML = "Channel order:";
  var input312 = HTMLElementUtils.createElement({
    kind: "input",
    id: "Spot_Inspector_order_channels",
    extraAttributes: {
      class: "form-text-input form-control",
      type: "text",
      value: JSON.stringify(Spot_Inspector._order_channels),
    },
  });
...
Spot_Inspector.loadImages = function (pathFormat) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      setTimeout(function () {
        Spot_Inspector.changeOrder(false);
        $("#Spot_Inspector_order_rounds")[0].value = JSON.stringify(
          Spot_Inspector._order_rounds
        );
        $("#Spot_Inspector_order_channels")[0].value = JSON.stringify(
          Spot_Inspector._order_channels
        );
      }, 500);
    },
...
Spot_Inspector.run = function () {
  if (Spot_Inspector._started) return;
  ...
  var click_handler = function (event) {
    if (event.quick) {
      ...
      if (markers.length > 0) {
        if (!Spot_Inspector._order_rounds) Spot_Inspector.changeOrder(false);
      }
