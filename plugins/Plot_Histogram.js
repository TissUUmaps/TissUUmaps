 ...
 Plot_Histogram.init = function (container) {
    ...
    row2=HTMLElementUtils.createRow({});
        col21=HTMLElementUtils.createColumn({"width":12});
            select211=HTMLElementUtils.createElement({"kind":"select","id":"Plot_Histogram_dataset","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label212=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"Plot_Histogram_dataset"} });
            label212.innerText="Select marker dataset";

    row7=HTMLElementUtils.createRow({});
        col71=HTMLElementUtils.createColumn({"width":12});
            select711=HTMLElementUtils.createElement({"kind":"select","id":"Plot_Histogram_histoKey","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label712=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"Plot_Histogram_histoKey"} });
    ...
    button111.addEventListener("click",(event)=>{
        ...
        interfaceUtils.getElementById("Plot_Histogram_dataset").dispatchEvent(event);
    });
    select211.addEventListener("change",(event)=>{
        ...
        interfaceUtils.cleanSelect("Plot_Histogram_histoKey");
        interfaceUtils.addElementsToSelect("Plot_Histogram_histoKey", dataUtils.data[Plot_Histogram._dataset]._csv_header);
        if (dataUtils.data[Plot_Histogram._dataset]._csv_header.indexOf(dataUtils.data[Plot_Histogram._dataset]._gb_col) > 0) {
            interfaceUtils.getElementById("Plot_Histogram_histoKey").value = dataUtils.data[Plot_Histogram._dataset]._gb_col;
            var event = new Event('change');
            interfaceUtils.getElementById("Plot_Histogram_histoKey").dispatchEvent(event);
        }
    });
...
Plot_Histogram.run = function () {
    ...
    tmapp["ISS_viewer"].addHandler("animation-finish", function animationFinishHandler(event){
        ...
        d3.selectAll(".region_histo").selectAll('polyline').each(function(el) {
            $(this).attr('stroke-width', regionUtils._polygonStrokeWidth / tmapp["ISS_viewer"].viewport.getZoom());
        });
        d3.selectAll(".region_histo").selectAll('circle').each(function(el) {
...
Plot_Histogram.releaseHandler = function (event) {
    ...
    regionobj.append('path').attr("d", regionUtils.pointsToPath([[Plot_Histogram._region]]))
        .attr("id", "path_UMAP")
        .attr('class', "region_histo")
        .attr('stroke-width', regionUtils._polygonStrokeWidth / tmapp["ISS_viewer"].viewport.getZoom())
    ...
    markerData[scalePropertyName] = new Float64Array(markerData[dataUtils.data[Plot_Histogram._dataset]["_X"]].length);
    var opacityPropertyName = "UMAP_Region_opacity"
    dataUtils.data[Plot_Histogram._dataset]["_opacity_col"] = opacityPropertyName;
    dataUtils.data[Plot_Histogram._dataset]["_opacity_col"] = opacityPropertyName;
    markerData[opacityPropertyName] = new Float64Array(markerData[dataUtils.data[Plot_Histogram._dataset]["_X"]].length);
...
Plot_Histogram.moveHandler = function (event) {
    if (event.buttons != 1 || Plot_Histogram._region == []){ //|| !event.shift) {
    ...
    if (Plot_Histogram._regionPixels.length > 1) {
        dis = distance(Plot_Histogram._regionPixels[Plot_Histogram._regionPixels.length-1], Plot_Histogram._regionPixels[Plot_Histogram._regionPixels.length-2])
        if (dis < 5) {
    ...
    var polyline = regionobj.append('polyline').attr('points', Plot_Histogram._region.map(function(x){return [x.x,x.y];}))
        .style('fill', 'none')
        .attr('stroke-width', regionUtils._polygonStrokeWidth / tmapp["ISS_viewer"].viewport.getZoom())
        .attr('stroke', '#ff0000').attr('class', "region_histo");
    return;
};
...
Plot_Histogram.analyzeRegion = function (points) {
    ...
    for (var codeIndex in allkeys) {
        ...
        if (visible) {
            var pointInBbox = Plot_Histogram.searchTreeForPointsInBbox(quadtree, x0, y0, x3, y3, options);
            ...
            for (var d of pointInBbox) {
                ...
                var key;
                if (regionUtils.globalPointInPath(x / imageWidth, y / imageWidth, regionPath, tmpPoint)) {
    ...
    for (var key in countsInsideRegion) {
        var hexColor;
        if (!Plot_Histogram._histoKey || Plot_Histogram._histoKey == dataUtils.data[dataset]._gb_col) {
        ...
        _histogram.push({ "key": key, "name": key, "count": countsInsideRegion[key], "color": hexColor});
    }
 ...
 *  Search for points inside a particular region */
 Plot_Histogram.searchTreeForPointsInBbox = function (quadtree, x0, y0, x3, y3, options) {    
...
Plot_Histogram.getHisto = function () {
    ...
    Plot_Histogram._plot = Plotly.newPlot( plotHistoView,
        [
            {
