 ...
 Points2Regions.init = function (container) {
    ...
    row2=HTMLElementUtils.createRow({});
        col21=HTMLElementUtils.createColumn({"width":12});
            select211=HTMLElementUtils.createElement({"kind":"select","id":"Points2Regions_dataset","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label212=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"Points2Regions_dataset"} });
            label212.innerText="Select marker dataset";

    row7=HTMLElementUtils.createRow({});
        col71=HTMLElementUtils.createColumn({"width":12});
            select711=HTMLElementUtils.createElement({"kind":"select","id":"Points2Regions_clusterKey","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label712=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"Points2Regions_clusterKey"} });
    ...
    row_nclusters=HTMLElementUtils.createRow({});
        col_nclusters1=HTMLElementUtils.createColumn({"width":12});
            label_nclusters12=HTMLElementUtils.createElement({"kind":"label","extraAttributes":{"class":"form-check-label","for":"Points2Regions_bboxSize"}});
            label_nclusters12.innerHTML="Number of clusters:";    
            var input_nclusters12=HTMLElementUtils.createElement({"kind":"input", "id":"Points2Regions_nclusters", "extraAttributes":{ "class":"form-text-input form-control", "type":"number", "value":Points2Regions._nclusters}});
    ...
    row_expression_threshold=HTMLElementUtils.createRow({});
        col_expression_threshold1=HTMLElementUtils.createColumn({"width":12});
            label_expression_threshold12=HTMLElementUtils.createElement({"kind":"label","extraAttributes":{"class":"form-check-label","for":"Points2Regions_bboxSize"}});
            label_expression_threshold12.innerHTML="Expression threshold:";    
            var input_expression_threshold12=HTMLElementUtils.createElement({"kind":"input", "id":"Points2Regions_expression_threshold", "extraAttributes":{ "class":"form-text-input form-control", "type":"number", "value":Points2Regions._expression_threshold}});
            
    input_expression_threshold12.addEventListener("change",(event)=>{
        Points2Regions._expression_threshold = parseInt(input_expression_threshold12.value);
    });

    row_normalize_order=HTMLElementUtils.createRow({});
        col_normalize_order1=HTMLElementUtils.createColumn({"width":12});
            label_normalize_order12=HTMLElementUtils.createElement({"kind":"label","extraAttributes":{"class":"form-check-label","for":"Points2Regions_bboxSize"}});
            label_normalize_order12.innerHTML="Normalize order:";    
            var input_normalize_order12=HTMLElementUtils.createElement({"kind":"input", "id":"Points2Regions_normalize_order", "extraAttributes":{ "class":"form-text-input form-control", "type":"number", "value":Points2Regions._normalize_order}});
    ...
    row_sigma=HTMLElementUtils.createRow({});
        col_sigma1=HTMLElementUtils.createColumn({"width":12});
            label_sigma12=HTMLElementUtils.createElement({"kind":"label","extraAttributes":{"class":"form-check-label","for":"Points2Regions_bboxSize"}});
            label_sigma12.innerHTML="Sigma:";    
            var input_sigma12=HTMLElementUtils.createElement({"kind":"input", "id":"Points2Regions_sigma", "extraAttributes":{ "class":"form-text-input form-control", "type":"number", "value":Points2Regions._sigma}});
    ...
    row_stride=HTMLElementUtils.createRow({});
        col_stride1=HTMLElementUtils.createColumn({"width":12});
            label_stride12=HTMLElementUtils.createElement({"kind":"label","extraAttributes":{"class":"form-check-label","for":"Points2Regions_bboxSize"}});
            label_stride12.innerHTML="Stride:";    
            var input_stride12=HTMLElementUtils.createElement({"kind":"input", "id":"Points2Regions_stride", "extraAttributes":{ "class":"form-text-input form-control", "type":"number", "value":Points2Regions._stride}});
    ...
    button111.addEventListener("click",(event)=>{
        ...
        interfaceUtils.getElementById("Points2Regions_dataset").dispatchEvent(event);
    });
    select211.addEventListener("change",(event)=>{
        ...
        interfaceUtils.cleanSelect("Points2Regions_clusterKey");
        interfaceUtils.addElementsToSelect("Points2Regions_clusterKey", dataUtils.data[Points2Regions._dataset]._csv_header);
        if (dataUtils.data[Points2Regions._dataset]._csv_header.indexOf(dataUtils.data[Points2Regions._dataset]._gb_col) > 0) {
            interfaceUtils.getElementById("Points2Regions_clusterKey").value = dataUtils.data[Points2Regions._dataset]._gb_col;
            var event = new Event('change');
            interfaceUtils.getElementById("Points2Regions_clusterKey").dispatchEvent(event);
        }
    });
    ...
    row_submit=HTMLElementUtils.createRow({});
        col_submit1=HTMLElementUtils.createColumn({"width":12});
            button_submit11=HTMLElementUtils.createButton({"extraAttributes":{ "class":"btn btn-primary mx-2"}})
...
Points2Regions.run = function () {
    ...
    if (typeof(csvFile) === 'object') {
        interfaceUtils.alert("This plugin can only run on datasets generated from buttons. Please convert your dataset to a button (Markers > Advanced Options > Generate button from tab)")
        return;
    }
    ...
    $.ajax(
        {
            ...
            error:function (data)
            {
                ...
                interfaceUtils.alert("Error during Points2Regions, check logs. This plugin only works on a pip installation of TissUUmaps, with the extra packages: pandas, sklearn, skimage")
            }
        }
    );
}
...
Points2Regions.loadRegions = function (data) {
    ...
    $(document.getElementById("regionClass-" + Points2Regions._region_name)).collapse("show");
