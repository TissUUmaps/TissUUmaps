 ...
 */
 ...
 ClassQC.init = function (container) {
    ...
    label611 = HTMLElementUtils.createElement({
      ...
      label611.innerHTML = "&nbsp;Show confusion matrix of two selected classifications";
    ...
    col71=HTMLElementUtils.createColumn({"width":12});
        label712=HTMLElementUtils.createElement({"kind":"label","extraAttributes":{"class":"form-check-label","for":"ClassQC_bboxSize"}});
        label712.innerHTML="Box size:";    
        var input712=HTMLElementUtils.createElement({"kind":"input", "id":"ClassQC_bboxSize", "extraAttributes":{ "class":"form-text-input form-control", "type":"number", "value":ClassQC._bboxSize}});
    ...
    row8=HTMLElementUtils.createRow({});
        col81=HTMLElementUtils.createColumn({"width":12});
            label812=HTMLElementUtils.createElement({"kind":"label","extraAttributes":{"class":"form-check-label","for":"ClassQC_bboxSize"}});
            label812.innerHTML="Figure size:";    
            var input812=HTMLElementUtils.createElement({"kind":"input", "id":"ClassQC_figureSize", "extraAttributes":{ "class":"form-text-input form-control", "type":"number", "value":ClassQC._figureSize}});
    ...
    row10=HTMLElementUtils.createRow({});
        col101=HTMLElementUtils.createColumn({"width":12});
            select1011=HTMLElementUtils.createElement({"kind":"select","id":"ClassQC_colormap","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label1012=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"ClassQC_colormap"} });
    ...
    row11=HTMLElementUtils.createRow({});
        col111=HTMLElementUtils.createColumn({"width":12});
            button1111=HTMLElementUtils.createButton({"extraAttributes":{ "class":"btn btn-primary mx-2"}})
    ...
    row12=HTMLElementUtils.createRow({});
        col121=HTMLElementUtils.createElement({"kind":"div","id":"ClassQC_matrix"});
        col131=HTMLElementUtils.createElement({"kind":"div","id":"ClassQC_legend"});
 ...
 ClassQC.display = function (pathFormat) {
    ...
    if (ClassQC._showMatrix) {
        ...
        var typeSet1 = Object.keys(dataUtils.data[ClassQC._dataset1]._groupgarden).sort();
        var typeSet2 = Object.keys(dataUtils.data[ClassQC._dataset2]._groupgarden).sort();
        // Make union of the two type sets into one:
        var typeSet = [...new Set([...typeSet1, ...typeSet2])];
        
        var types_dataset1 = dataUtils.data[ClassQC._dataset1]._processeddata[ClassQC._dataset1_column];
        var types_dataset2 = dataUtils.data[ClassQC._dataset2]._processeddata[ClassQC._dataset2_column];
        dataUtils.data[ClassQC._dataset1]._processeddata["ClassQC_confusion"] = Array(types_dataset1.length);
        ...
        for (var typeIndex in types_dataset1) {
            ...
            dataUtils.data[ClassQC._dataset1]._processeddata["ClassQC_confusion"][typeIndex] = types_dataset1[typeIndex] + "_" + types_dataset2[typeIndex];
...
ClassQC.displayConfusion = function (_matrix_header, _matrix) {
  ...
  var layout = {
    ...
    yaxis: {
      ...
      ticktext: _matrix_header.map(function (type) {
        try {
            typecolor = document.getElementById(ClassQC._dataset1 +"_" + type + "_color").value;
        } catch (error) {
            typecolor = document.getElementById(ClassQC._dataset2 +"_" + type + "_color").value;
        }
        return "<span style='font-weight:bold;color:"+typecolor+"'>███</span>";
      }),
      ticks:"",
      tickangle:90,
      title: {
    ...
    //   "scaleanchor":"x",
      ...
      ticktext: _matrix_header.slice().reverse().map(function (type) {
        try {
            typecolor = document.getElementById(ClassQC._dataset2 +"_" + type + "_color").value;
        } catch (error) {
            typecolor = document.getElementById(ClassQC._dataset1 +"_" + type + "_color").value;
  ...
  var maxRowVal = _matrix.map(function(row){ return Math.max.apply(Math, row); });
  ...
  document.getElementById("ClassQC_matrix").on('plotly_click', function(data){
    ...
    var confusions = dataUtils.data[ClassQC._dataset1]._processeddata["ClassQC_confusion"];

    for (var d in confusions) {
        markerData[opacityPropertyName][d] = dataUtils.data[ClassQC._dataset1]._processeddata["ClassQC_confusion"][d] == clicked_y + "_" + clicked_x;
    }
    glUtils.loadMarkers(ClassQC._dataset1);
...
ClassQC.run = function () {
    ...
    var click_handler = function (event) {
        if (event.quick) {
            ...
            var bbox = [Math.round(normCoords.x - ClassQC._bboxSize/2), Math.round(normCoords.y - ClassQC._bboxSize/2), ClassQC._bboxSize, ClassQC._bboxSize];
    ...
    if (ClassQC.ISS_mouse_tracker == undefined) {
        ...
        ClassQC.ISS_mouse_tracker = tmapp["ISS_viewer"].addHandler('canvas-click', (event) => {
            click_handler(event)
        });
    }
}
