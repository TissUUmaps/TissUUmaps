...
function _range(start,end){  start = start || 1; return end >= start ? _range(start,end-1).concat(end) : []; }

tests = []
for (h5 of [false, true]) {
    for (p of _range(6,36)) {
        for (replicate of _range(1,10)) {
            tests.push({
                _useH5:h5,
                p: p,
                replicate: replicate
...
const queryString = window.location.search;
const urlParams = new URLSearchParams(queryString);
var test_index = parseInt(urlParams.get('test_index'))
if (isNaN(test_index)) {
    test_index = 0;
...
async function waitForDraw() {
    return new Promise(function(resolve, reject) {
      document.addEventListener('glUtilsDraw', function(e) {
        resolve(e); // done
      }, {once:true});
    });
}
...
async function benchmark () {
    ...
    _useH5 = tests[test_index]._useH5
    p = tests[test_index].p;
    replicate = tests[test_index].replicate;
    console.log(p, replicate, _useH5)
    
    await new Promise(r => setTimeout(r, 3000));
    dataUtils._quadtreesEnabled = false;
    let i = Math.pow(2,p);
    
    if (!_useH5) {
        start = performance.now()
        var options = {
            "path": "benchmark_data/"+i.toString()+".csv",
            "comment": "",
            "title": "Download data",
            "hideSettings": true,
            "autoLoad": false,
            ...
            "name": i.toString()+".csv",
            "expectedHeader": {
                "X": "global_x","Y": "global_y","gb_col": "gene",
            },
            "expectedRadios": {
                "cb_col": false,
                "cb_gr": true,
        ...
        interfaceUtils._mGenUIFuncs.deleteTab("JUPOVH")
    }
    else {
        start = performance.now()
        var options = {
            "path": "benchmark_data/"+i.toString()+".h5",
            "comment": "",
            "title": "Download data",
            "hideSettings": true,
            "autoLoad": false,
            ...
            "name": i.toString()+".csv",
            "expectedHeader": {
                "X": "/obs/global_x","Y": "/obs/global_y","gb_col": "/obs/gene",
            },
            "expectedRadios": {
                "cb_col": false,
                "cb_gr": true,
        ...
        dataUtils._hdf5Api.worker.postMessage({action: "clear"});
        dataUtils._hdf5Api.status = {};
    ...
    var url = "TissUUmaps3.tmap?path=./&test_index=" + (test_index+1);    
    var a = window.open(url, '_self');
}
