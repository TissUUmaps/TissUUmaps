...
def bundleHTMLtoOneFile(htmlFile):
    ...
    dirname = os.path.dirname(htmlFile)
    ...
    for tag in soup.find_all("link", href=True):
        if tag.has_attr("href"):
            file_text = Path(os.path.join(dirname, tag["href"])).read_text(
                encoding="utf-8"
            )
    ...
    for tag in soup.find_all("script", src=True):
        if tag.has_attr("src"):
            file_text = Path(os.path.join(dirname, tag["src"])).read_text(
                encoding="utf-8"
            )
    ...
    for tag in soup.find_all("img", src=True):
        if tag.has_attr("src"):
            file_content = Path(os.path.join(dirname, tag["src"])).read_bytes()
