 ...
 Feature_Space_3D.init = function (container) {
    ...
    row2=HTMLElementUtils.createRow({});
        col21=HTMLElementUtils.createColumn({"width":12});
            select211=HTMLElementUtils.createElement({"kind":"select","id":"Feature_Space_3D_dataset","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label212=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"Feature_Space_3D_dataset"} });
            label212.innerText="Select marker dataset";

    row3=HTMLElementUtils.createRow({});
        col31=HTMLElementUtils.createColumn({"width":12});
            select311=HTMLElementUtils.createElement({"kind":"select","id":"Feature_Space_3D_UMAP1","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label312=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"Feature_Space_3D_UMAP1"} });
            label312.innerText="Select Dimension 1";

    row4=HTMLElementUtils.createRow({});
        col41=HTMLElementUtils.createColumn({"width":12});
            select411=HTMLElementUtils.createElement({"kind":"select","id":"Feature_Space_3D_UMAP2","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label412=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"Feature_Space_3D_UMAP2"} });
            label412.innerText="Select Dimension 2";

    row5=HTMLElementUtils.createRow({});
        col51=HTMLElementUtils.createColumn({"width":12});
            select511=HTMLElementUtils.createElement({"kind":"select","id":"Feature_Space_3D_UMAP3","extraAttributes":{"class":"form-select form-select-sm","aria-label":".form-select-sm"}});
            label512=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{"for":"Feature_Space_3D_UMAP3"} });
            label512.innerText="Select Dimension 3";
    
    row6=HTMLElementUtils.createRow({});
        col61=HTMLElementUtils.createColumn({"width":12});
            var input611=HTMLElementUtils.createElement({"kind":"input", "id":"Feature_Space_3D_ortho","extraAttributes":{"class":"form-check-input","type":"checkbox"}});
            label611=HTMLElementUtils.createElement({"kind":"label", "extraAttributes":{ "for":"Feature_Space_3D_ortho" }});
    ...
    row7=HTMLElementUtils.createRow({});
        col71=HTMLElementUtils.createColumn({"width":12});
            button711=HTMLElementUtils.createButton({"extraAttributes":{ "class":"btn btn-primary mx-2"}})
    ...
    button111.addEventListener("click",(event)=>{
        ...
        interfaceUtils.getElementById("Feature_Space_3D_dataset").dispatchEvent(event);
    });
    ...
    select211.addEventListener("change",(event)=>{
        ...
        interfaceUtils.cleanSelect("Feature_Space_3D_UMAP1");
        interfaceUtils.addElementsToSelect("Feature_Space_3D_UMAP1", dataUtils.data[Feature_Space_3D._dataset]._csv_header);
        interfaceUtils.cleanSelect("Feature_Space_3D_UMAP2");
        interfaceUtils.addElementsToSelect("Feature_Space_3D_UMAP2", dataUtils.data[Feature_Space_3D._dataset]._csv_header);
        interfaceUtils.cleanSelect("Feature_Space_3D_UMAP3");
        interfaceUtils.addElementsToSelect("Feature_Space_3D_UMAP3", dataUtils.data[Feature_Space_3D._dataset]._csv_header);
        if (dataUtils.data[Feature_Space_3D._dataset]._csv_header.indexOf("UMAP1") > 0) {
            interfaceUtils.getElementById("Feature_Space_3D_UMAP1").value = "UMAP1";
            var event = new Event('change');
            interfaceUtils.getElementById("Feature_Space_3D_UMAP1").dispatchEvent(event);
        }
        if (dataUtils.data[Feature_Space_3D._dataset]._csv_header.indexOf("UMAP2") > 0) {
            interfaceUtils.getElementById("Feature_Space_3D_UMAP2").value = "UMAP2";
            var event = new Event('change');
            interfaceUtils.getElementById("Feature_Space_3D_UMAP2").dispatchEvent(event);
        }
        if (dataUtils.data[Feature_Space_3D._dataset]._csv_header.indexOf("UMAP3") > 0) {
            interfaceUtils.getElementById("Feature_Space_3D_UMAP3").value = "UMAP3";
            var event = new Event('change');
            interfaceUtils.getElementById("Feature_Space_3D_UMAP3").dispatchEvent(event);
        }
    });
...
Feature_Space_3D.run = function () {
    ...
    var Feature_Space_3D_Control = document.getElementById("Feature_Space_3D_Control");
    ...
    Plotly.newPlot(Feature_Space_3D_Control, data, layout, {responsive: true,displayModeBar: false});
    
    if ( interfaceUtils.Feature_Space_3D_toggleRightPanel === undefined) {
        interfaceUtils.Feature_Space_3D_toggleRightPanel = interfaceUtils.toggleRightPanel;
    ...
    if (glUtils.Feature_Space_3D_draw === undefined) {
        ...
        glUtils.Feature_Space_3D_updateColorLUTTexture = glUtils._updateColorLUTTexture;
    ...
    for (let i = 0, index = 0; i < numPoints; ++i) {
        ...
        if (markers3D[key] === undefined) {
            ...
            markers3D[key] = {
                "x":[],"y":[],"z":[],"mode":"markers",
                "marker":{
                    "color":hexColor, size:2, symbol:shape,opacity:visible?1:0.000001
                },
                "type":"scatter3d",
                "name":key,
                "meta":key.toString(),
